import React ,{useState} from 'react'
import { useDispatch, useSelector } from 'react-redux'
import { motion } from "framer-motion"
import { RiRobot3Fill } from "react-icons/ri";
import { GiTwoCoins } from "react-icons/gi";
import { FaUserAstronaut } from "react-icons/fa";
import { useNavigate } from 'react-router-dom';
import { MdOutlineLogout } from "react-icons/md";
import api from '../lib/axios.js'
import AuthModel from './AuthModel.jsx';
import { setUserData } from '../lib/userSlice.js';

const Navbar = () => {
    const {userData} = useSelector((state)=> state.user)
    const [showCredit, setShowCredit] = useState(false)
    const [showUser, setShowUser] = useState(false)
    const [showAuth, setShowAuth] = useState(false)
    const navigate = useNavigate()
    const dispatch = useDispatch()

    const handleLogout = async ()=>{
        try {
            await api.get("/auth/logout")
            dispatch(setUserData(null))
            setShowCredit(false)
            setShowUser(false)
            navigate("/")
        } catch (error) {
            console.log("errro in logout " , error)
        }
    }

  return (
    <div className=' bg-[#f3f3f3] flex justify-center px-4 pt-6 '>
      <motion.div 
      initial={{opacity:0 , y:-40}}
       animate={{opacity:1 , y:0}}
       translate={{duration:0.3}}
       className='w-full max-w-6xl bg-white rounded-[24px] shadow-sm border border-gray-200 px-8 py-4 flex justify-between  items-center relative  '
      >

        <div className='flex items-center gap-3 cursor-pointer'>
            <div className='bg-black text-white p-2 rounded-lg'>
                <RiRobot3Fill size={18} />
            </div>
            <h1 className='font-semibold hidden md:block text-lg '>Interview.AI</h1>
        </div>

        <div className='flex items-center gap-6 relative'>
            {/* credit */}
            <div className='relative'>
                <button onClick={()=>{
                    if(!userData){
                        setShowAuth(true)
                    }
                    if(userData){
                    setShowCredit(!showCredit)
                    setShowUser(false)
                    }
                }}
                className='flex items-center gap-2 bg-gray-100 px-4 py-2 rounded-full text-md hover:bg-gray-200 transition'>
                    <GiTwoCoins size={20}/>
                    {userData?.credits || 0}
                </button>
                {showCredit && (
                    <div className='absolute right-[-50px] mt-3 w-64 bg-white shadow-xl border border-gray-200 rounded-xl p-5 z-50 '>
                        <p className='text-sm text-gray-600 mb-4'>Need more credits to continue interviews? </p>
                        <button onClick={()=>navigate("/pricing")}
                        className='w-full bg-black text-white py-2 rounded-lg text-sm' >Buy more credits</button>
                    </div>
                )}
            </div>
              {/* user */}
            <div className='relative'>
                <button onClick={()=>{
                     if(!userData){
                        setShowAuth(true)
                    }
                     if(userData){
                     setShowCredit(false)
                     setShowUser(!showUser)
                    }
                }}
                className='w-9 h-9 bg-black text-white rounded-full flex items-center  justify-center font-semibold '>
                    {userData? userData?.name?.slice(0,1).toUpperCase() : <FaUserAstronaut /> }
                </button>
                {showUser && (
                    <div className='absolute right-0 mt-1 w-48  bg-white shadow-xl border border-gray-200 rounded-xl p-3 z-50 '>
                        <p className='text-md text-blue-500 font-medium mb-1 cursor-pointer ' >{userData?.name}</p>
                        <button onClick={()=>navigate("/history")}
                        className='w-full cursor-pointer text-left text-md py-2 hover:text-black text-gray-600'>Interview History</button>

                        <button onClick={handleLogout}
                        className='w-full cursor-pointer text-left text-sm py-2 flex items-center gap-2 text-red-500' >
                           <MdOutlineLogout size={16} /> Logout
                        </button>
                    </div>
                )}
            </div>

        </div>
        </motion.div>
        {showAuth && <AuthModel onClose={()=>setShowAuth(false)} />}
    </div>
  )
}

export default Navbar
