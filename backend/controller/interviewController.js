...
export const generateQuestion = async (req,res)=>{
    ...
    if(!aiResponse || !aiResponse.trim()){
        return res.status(500).json({message:"AI returned empty prompt"})
    }
    ...
    res.json({
      ...
      questions: interview.questions
        ...
        console.log("error in generate questions" , error)
...
export const submitAnswer = async (req,res)=>{
  try {
    ...
    if(timeTaken > interview.timeLimit){
          question.score = 0;
    ...
    console.log("error in generate answer" , error)
    return res.status(500).json({message:"Internal server error"})
...
export const getMyInterview = async (req,res)=>{
    try {
        const interviews = await interviewModel.find({userId: req.userId}).sort({createdAt:-1}).select("role experience mode finalScore status createdAt")
